...
- **Working tree:** `git status -sb`. Read `src/config/git-workflow.json`, then apply `.cursor/rules/git-workflow/RULE.md` § Mode-aware branch gate (read config **before** treating the current branch as allowed/forbidden).
