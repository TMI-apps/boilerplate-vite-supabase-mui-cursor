...
- [ ] **Implementation:** App-code work follows `.cursor/rules/git-workflow/RULE.md` § Mode-aware branch gate (read `src/config/git-workflow.json` first). Route to `implement` from a branch the gate allows (`feature/*` / `fix/*` in Model A; `develop` in Model B).
